...
<div class="navbar-custom">
            ...
            <div class="dropdown-menu dropdown-menu-end dropdown-menu-animated topbar-dropdown-menu">
                @foreach (config('app.available_locales') as $locale)
                    <a href="{{ route('lang.switch', ['locale' => $locale]) }}" class="dropdown-item notify-item">
                        <img src="{{ asset("assets/images/flags/$locale.jpg") }}" alt="{{ strtoupper($locale) }}" class="me-1" height="12">
                        @if ($locale === 'fr')
                            Français
                        @else
                            English
    ...
    @if (request()->isMobile())
        <a href="{{ route('home') }}" class="btn btn-info btn-sm ms-2 d-inline-flex d-sm-none align-items-center" title="{{ __('messages.home') }}">
